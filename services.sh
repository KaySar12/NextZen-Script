...
install() {
  ...
  if curl -fsSL "https://setup.nextzen.cstsoft.net/$version/install.sh" >/dev/null 2>&1; then
    curl -fsSL "https://setup.nextzen.cstsoft.net/$version/install.sh" | ${sudo_cmd} bash
  else
